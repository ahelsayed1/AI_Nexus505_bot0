...
# ضيف السطرين دول في أول ملف ai_manager.py
...
class SmartAIManager:
    ...
    # ========== Google Veo 3.1 (النسخة النهائية - Service Account Auth) ==========
    
    async def _generate_video_veo(self, model_name: str, prompt: str, image_url: str = None) -> str:
        """
        توليد فيديو باستخدام Google Veo 3.1 مع التوثيق عبر Service Account
        """
        try:
            logger.info(f"🎬 Google Veo: {model_name}...")
            
            # التأكد من وجود ملف الاعتماد JSON
            creds_json = os.getenv("GOOGLE_CREDENTIALS_JSON")
            if not creds_json:
                logger.warning("⚠️ لم يتم العثور على GOOGLE_CREDENTIALS_JSON، استخدام Luma كبديل")
                luma_config = self.providers[Provider.LUMA]
                if luma_config.enabled:
                    return await self._generate_video_luma(prompt, image_url)
                raise Exception("Service Account JSON مفقود، لا يمكن تشغيل Veo")
            
            if not os.path.exists("downloads"):
                os.makedirs("downloads")

            # 1. توليد Token من ملف الـ JSON
            ...
            access_token = creds.token # هذا هو المفتاح السحري

            # 2. الرابط (بدون API Key في الرابط)
            ...
            timeout_seconds = 40 if "fast" in model_name.lower() else 120
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(url, headers=headers, json=payload) as response:
                    ...
                    if video_b64:
                        ...
                        raise Exception("ملف الفيديو مفقود في الرد")
