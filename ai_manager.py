...
class SmartAIManager:
    ...
    # ========== Google Veo 3.1 - (الكود المعدل لإصلاح خطأ 401) ==========
    ...
    async def _generate_video_veo(self, model_name: str, prompt: str, image_url: str = None) -> str:
        ...
        try:
            ...
            # === التعديل هنا: وضعنا المفتاح في الرابط بدلاً من الهيدر ===
            url = f"https://{self.veo_location}-aiplatform.googleapis.com/v1/projects/{self.veo_project_id}/locations/{self.veo_location}/publishers/google/models/{model_name}:predict?key={self.veo_api_key}"
            
            # === التعديل هنا: حذفنا Authorization Bearer ===
            headers = {
                "Content-Type": "application/json"
            }
